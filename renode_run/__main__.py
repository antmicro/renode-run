...
def download_command(artifacts_path: artifacts_path_annotation = default_renode_artifacts_dir,
    ...
    if artifacts_path == default_renode_artifacts_dir and saved_artifacts_path != default_renode_artifacts_dir:
        artifacts_path = saved_artifacts_path
...
def demo_command(board: Annotated[str, typer.Option("-b", "--board", help='board name, as listed on https://zephyr-dashboard.renode.io')],
                 binary: Annotated[str, typer.Argument(help='binary name, either local or remote')] = None,
...
def exec_command(artifacts_path: artifacts_path_annotation = default_renode_artifacts_dir):
...
def parse_artifacts_path(ctx: typer.Context, artifacts_path: artifacts_path_annotation = default_renode_artifacts_dir):
    # For backward compatibility we're allowing to pass artifacts_path before specifying the command
    global saved_artifacts_path
    saved_artifacts_path = artifacts_path
