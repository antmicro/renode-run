...
test_downloading_present_renode_version()
{
  local RENODE_VERSION=1.16.1+20260302gita3bdf4a87
  renode-run download $RENODE_VERSION
  assert_artifact_exists "$DEFAULT_INSTALL_PATH" "renode-$RENODE_VERSION"
...
test_downloading_present_renode_version_in_custom_location()
{
  ...
  assert_artifact_exists "$TEST_DOWNLOAD_PATH" "renode-$RENODE_VERSION"
...
test_downloading_different_renode_version()
{
  ...
  assert_artifact_exists "$DEFAULT_INSTALL_PATH" "renode-$RENODE_VERSION1"
  # Presence of one version should not prevent downloading another.
  renode-run download $RENODE_VERSION2
  assert_artifact_exists "$DEFAULT_INSTALL_PATH" "renode-$RENODE_VERSION2"
}
...
test_behavior_on_external_install_delete()
{
  ...
  renode-run download $RENODE_VERSION
  assert_artifact_exists "$DEFAULT_INSTALL_PATH/renode-$RENODE_VERSION" "renode"
  rm -rf "$DEFAULT_INSTALL_PATH/renode-$RENODE_VERSION"
...
test_reinstall_updates_default_version()
{
  ...
  local RENODE_VERSION2=1.16.1+20260515gitac6335d02
  renode-run download $RENODE_VERSION1
  assert_artifact_exists "$DEFAULT_INSTALL_PATH/renode-$RENODE_VERSION1" "renode"
  renode-run download $RENODE_VERSION2
  assert_artifact_exists "$DEFAULT_INSTALL_PATH/renode-$RENODE_VERSION2" "renode"
  renode-run download $RENODE_VERSION1
  assert_artifact_exists "$DEFAULT_INSTALL_PATH/renode-$RENODE_VERSION1" "renode"
